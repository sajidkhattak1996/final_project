              ...
              <!--============ended=============================================================================================================================-->
                ...
                <table id="example1" class="table table-straped ">
                    ...
                    <tbody>
                      ...
                      <?php
                          $ass_record=mysqli_query($con ,"SELECT quize.Q_id,quize.q_topic,quize.q_date,quize.qt_marks FROM quize INNER JOIN quiz_record ON quiz_record.Q_id=quize.Q_id WHERE Class_id='$cid' GROUP BY Q_id");
                          if (mysqli_num_rows($ass_record)>0) {
                            ...
                            while ($r=mysqli_fetch_assoc($ass_record)) { $i++;
                                ...
                                <tr id="row<?php echo $i; ?>">
                                  <form  method="post" name="f<?php echo $i; ?>" id="f" action="edit_Quize_topic_processing.php?id=<?php  echo $r['Q_id']; ?>&loop=<?php echo $i; ?>" >
                                  <td> <input type="text" name="topic<?php echo $i; ?>" id="r1<?php echo $i; ?>" value="<?php  echo $r['q_topic'];  ?>"   readonly onchange="Fn_change(<?php echo $i; ?>)" style="border: none" required pattern="^[a-zA-Z][a-zA-Z0-9-_\s.]{2,50}$" title="Quize Topic be start with Alphabit & between 3 to 50 character long"> </td>
                                  ...
                                  <td> <input type="number" name="total_marks<?php echo $i; ?>" id="r4<?php echo $i; ?>" value="<?php  echo $r['qt_marks']; ?>" readonly onchange="Fn_change(<?php echo $i; ?>)" style="border: none" min="<?php  echo $r['qt_marks']; ?>" max="5000"> </td>
                                  <style media="screen">
                                  ...
                                  <td id="dd<?php echo $i; ?>">
                                    ...
                                    <a href="delete_Quize_topic_processing.php?i=<?php echo $i; ?>&id=<?php echo $r['Q_id']; ?>&cid=<?php echo $cid; ?>" id="conf_del<?php echo $i; ?>" style="display:none" > <button type="button" name="button" class="btn btn-sm btn-danger">conform</button> </a>
                                  </td>
                                </form>
                                </tr>
                                <?php
                            ...
                            <tr>
                              <td colspan="5" class="text-center alert alert-warning">No Quize Yet. </td>
                            </tr>
                            <?php
                          }
                       ?>
