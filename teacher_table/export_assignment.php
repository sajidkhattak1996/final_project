<?php
if (isset($_POST['export_assignment'])) {
  include('../db_connection.php');
  $cid=$_POST['export_assignment'];
  ...
  fputcsv($output, array('Class NO', 'Name', 'Assignment Topic', 'Assignment Date','Obtained Marks','Total Marks'));
  fputcsv($output, array("------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"));
            ...
            $exe_attd=mysqli_query($con ,$stmt_check_attendence);
                if (mysqli_num_rows($exe_attd)>0 ) {
                  while ($std_id=mysqli_fetch_assoc($exe_check)) {
                    ...
                    if ($assi_row>0) {
                      $count=0;
                      while ($ass_result=mysqli_fetch_assoc($assignment)) {
                        if ($count<1) { $count++;
